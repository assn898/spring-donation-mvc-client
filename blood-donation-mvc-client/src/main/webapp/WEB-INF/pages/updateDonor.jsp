...
<html>
<head>
<title>Update Donor - ID:<c:out value="${updateDonorObject.donorId}"></c:out></title>
...
<section class="newFormSection">
	<h2 class="title">Update Donor - ID:<c:out value="${updateDonorObject.donorId}"></c:out></h2>
	<small class="mb-1" style="display: block; text-align: center; font-weight:600;">Update any detail as per requirement. Check all fields before updating!</small>
	<div class="container">
		<form:form action="submitUpdate" method="POST" modelAttribute="updateDonorObject">
		<div class="newForm row">
		<div style="display:none;">
			<p>${updateDonorObject.donorId}</p>
			<form:hidden path="donorId" />
			<label for="">Donor ID</label>
		</div>
		<div class="form-field col-lg-12">
			<form:input path="donorName" type="text" class="input-text" id="donorName"/>
			<label for="donorName" class="label">Donor Name</label>
		</div>
		<div class="form-field col-lg-5">
			<form:input path="dateOfBirth" type="date" id="dateOfBirth" class="input-text"/>
			<label for="dateOfBirth" class="label">Date of Birth</label>
		</div>
		<div class="form-field col-lg-3">
			<form:input path="donorAge" id="donorAge" class="input-text" />
			<label for="donorAge" class="label">Age</label>
		</div>
		<div class="form-field col-lg-4">
			<form:select path="donorGender" class="input-text" id="donorGender">
		...
		<div class="form-field col-lg-6">
			<form:input path="location" class="input-text" id="location"/>
			<label for="location" class="label">Location</label>
		</div>
		<div class="form-field col-lg-5">
			<form:input path="phoneNumber" class="input-text" id="phoneNumber"/>
			<label for="phoneNumber" class="label">Phone Number</label>
		</div>
		<div class="form-field col-lg-7">
			<form:input path="emailAddr" class="input-text" id="emailAddr"/>
		...
		<div class="form-field col-lg-12" title="Approximate Date of Previous Blood Donation">
			<form:input path="lastDonated" type="date" class="input-text" id="lastDonated"/>
			<label for="lastDonated" class="label">Date of Last Donation</label>
		</div>
		
		<div style="display:none">
			<label for="">Donation Camp </label>
			<form:input path="donationCampName" type="hidden"/>
		</div>
		
		<div class="form-field col-lg-12">
			<input class="submit-btn mx-auto" type="submit" value="Update Donor" />
		</div>
		</div>
		</form:form>
	</div>
